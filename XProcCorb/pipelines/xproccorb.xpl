...
<p:declare-step 
	...
	<p:declare-step type="xpc:load-query">
		...
		<p:group>
			...
			<p:add-attribute match="/c:request" attribute-name="href">
				<p:with-option name="attribute-value" select="/c:request/ml:config/node()[local-name() = concat($query, '-query')]/@href"/>
			</p:add-attribute>
	...
	<p:declare-step type="xpc:get-content-uris">
		<p:documentation>Executes the source XQuery against the specified database which returns a list of document URIs.</p:documentation>
		<p:input port="source"/>
	...
	<p:group>
		<p:documentation>Main pipeline that loads queries, executes them against the chosen content in the named database and returns the results of those queries/transforms.</p:documentation>
