...
(:
 : This function constructs a task to be invoked at daily intervals. : @param $TASK_PATH
 : @param $TASK_ROOT
 : @param $TASK_PERIOD
...
try {
	admin:group-daily-scheduled-task(xs:string($TASK_PATH), xs:string($TASK_ROOT), xs:positiveInteger($TASK_PERIOD), xs:time($TASK_START_TIME), xs:unsignedLong($TASK_DATABASE), xs:unsignedLong($TASK_MODULES), xs:unsignedLong($TASK_USER), xs:unsignedLong($TASK_HOST))
} catch($error) {
	$error
}