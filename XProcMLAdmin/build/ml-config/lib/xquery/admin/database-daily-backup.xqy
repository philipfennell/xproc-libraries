...
(:
 : This function constructs a daily scheduled database backup. : @param $BACKUP_DIR
 : @param $BACKUP_PERIOD
 : @param $START_TIME
 : @param $MAX_BACKUPS
...
try {
	admin:database-daily-backup(xs:string($BACKUP_DIR), xs:positiveInteger($BACKUP_PERIOD), xs:time($START_TIME), xs:unsignedLong($MAX_BACKUPS), xs:boolean($BACKUP_SECURITY_DB), xs:boolean($BACKUP_SCHEMAS_DB), xs:boolean($BACKUP_TRIGGERS_DB), xs:boolean($INCLUDE_REPLICAS))
} catch($error) {
	$error
}