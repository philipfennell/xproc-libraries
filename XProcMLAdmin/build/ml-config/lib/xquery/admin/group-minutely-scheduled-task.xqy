...
(:
 : This function constructs a task to be invoked at intervals defined in terms of minutes. : @param $TASK_PATH
 : @param $TASK_ROOT
 : @param $TASK_PERIOD
 : @param $TASK_DATABASE
 : @param $TASK_MODULES
...
try {
	admin:group-minutely-scheduled-task(xs:string($TASK_PATH), xs:string($TASK_ROOT), xs:positiveInteger($TASK_PERIOD), xs:unsignedLong($TASK_DATABASE), xs:unsignedLong($TASK_MODULES), xs:unsignedLong($TASK_USER), xs:unsignedLong($TASK_HOST))
} catch($error) {
	$error
}