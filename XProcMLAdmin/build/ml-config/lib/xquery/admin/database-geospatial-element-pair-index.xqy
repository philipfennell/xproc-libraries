...
(:
 : This function constructs a geospatial element pair index specification. : @param $PARENT_NAMESPACE
 ...
 : @return element(db:geospatial-element-pair-index)
 :)

try {
	admin:database-geospatial-element-pair-index(xs:string($PARENT_NAMESPACE), xs:string($PARENT_LOCALNAME), xs:string($LATITUDE_NAMESPACE), xs:string($LATITUDE_LOCALNAME), xs:string($LONGITUDE_NAMESPACE), xs:string($LONGITUDE_LOCALNAME), xs:string($COORDINATE_SYSTEM), xs:boolean($RANGE_VALUE_POSITIONS))
} catch($error) {
	$error
}